...
const createPayment = async (req, res) => {
  try {
    ...
    const enrollmentCheck = await EnrollmentServiceClient.checkPaymentRequired(enrollmentId, token);
    ...
    if (!enrollment.canProcessPayment) {
      return res.status(400).json({
        success: false,
        error: 'No se puede procesar el pago para esta inscripción en su estado actual'
      });
    }
